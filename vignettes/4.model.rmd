...
## Model Evaluation
The model selected for final prediction on 2016 data would be that which performed well on squared error while meeting Bayesian regression inference conditions, specifically residual versus predicted homoscedacticity. Here, the top four models are evaluated in terms of residual versus fitted distributions, the parameter coefficients, and their prediction intervals. All four priors generated similar distributions of residuals vs-a-vis fitted values. Residuals concentration tended to coincide with the proximity of fitted values. Three same three observations were influential and potentially outliers.  
