...
```{r trim}
trimmedData  <- preprocess(movies, trim = TRUE)
yXTrimmed <- trimmedData %>% select(-title)
save(yXTrimmed, file = "./analysis/yXTrimmed.Rdata")
```


```{r bmaTrim}
modelsTrim <- bma(yX = yXTrimmed)
save(modelsTrim, file = "./analysis/modelsTrim.Rdata")
```

```{r bmaAnalysisTrim}
if (run == TRUE)  {
  analysisTrim <- bmaAnalysis(models = modelsTrim)
  save(analysisTrim, file = "./analysis/analysisTrim.Rdata")
}
```

```{r bmaPerformanceTrim}
trials <- 2
if (run == TRUE) {
  performanceTrim <- bmaPerformance(yX = yXTrimmed, trials = trials) 
  save(performanceTrim, file = "./analysis/performanceTrim.Rdata")
}
```

```{r bmaPerformanceReportTrim}
reportTrim <- bmaPerformanceReport(performance = performanceTrim)
save(reportTrim, file = "./analysis/reportTrim.Rdata")
```

```{r bmaEvalTrim}
evalTrim <- bmaEvaluation(mList = modelsTrim, candidates = reportTrim$best)
save(evalTrim, file = "./analysis/evalTrim.Rdata")
```
